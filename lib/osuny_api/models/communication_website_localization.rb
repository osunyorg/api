  ...
  class CommunicationWebsiteLocalization
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `OsunyApi::CommunicationWebsiteLocalization` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `OsunyApi::CommunicationWebsiteLocalization`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'social_email')
        self.social_email = attributes[:'social_email']
      end
    ...
    def hash
      [id, name, published, published_at, social_email, social_facebook, social_github, social_instagram, social_linkedin, social_mastodon, social_peertube, social_tiktok, social_vimeo, social_x, social_youtube, created_at, updated_at].hash
    end
